...
func (*Zone) Descriptor() ([]byte, []int) {
	return fileDescriptor_42516036701bc38d, []int{0}
}
...
func (*ZoneList) Descriptor() ([]byte, []int) {
	return fileDescriptor_42516036701bc38d, []int{1}
}
...
func init() { proto.RegisterFile("etc.zones.proto", fileDescriptor_42516036701bc38d) }

var fileDescriptor_42516036701bc38d = []byte{
	// 172 bytes of a gzipped FileDescriptorProto
	,
}
...
func (m *Zone) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	if len(m.State) > 0 {
		i -= len(m.State)
		copy(dAtA[i:], m.State)
		i = encodeVarintEtcZones(dAtA, i, uint64(len(m.State)))
		i--
		dAtA[i] = 0x1a
	}
	if len(m.Name) > 0 {
		i -= len(m.Name)
		copy(dAtA[i:], m.Name)
		i = encodeVarintEtcZones(dAtA, i, uint64(len(m.Name)))
		i--
		dAtA[i] = 0x12
	}
	if m.ZoneID != 0 {
		i = encodeVarintEtcZones(dAtA, i, uint64(m.ZoneID))
		i--
		dAtA[i] = 0x8
	}
		...
		for iNdEx := len(m.Zones) - 1; iNdEx >= 0; iNdEx-- {
			{
				...
				i -= size
				i = encodeVarintEtcZones(dAtA, i, uint64(size))
			}
			i--
			dAtA[i] = 0xa
		}
...
func encodeVarintEtcZones(dAtA []byte, offset int, v uint64) int {
	offset -= sovEtcZones(v)
	base := offset
	for v >= 1<<7 {
...
func (m *Zone) Size() (n int) {
	...
	_ = l
	if m.ZoneID != 0 {
		n += 1 + sovEtcZones(uint64(m.ZoneID))
	}
	l = len(m.Name)
	if l > 0 {
		n += 1 + l + sovEtcZones(uint64(l))
	}
	l = len(m.State)
	if l > 0 {
		n += 1 + l + sovEtcZones(uint64(l))
...
func sovEtcZones(x uint64) (n int) {
	return (math_bits.Len64(x|1) + 6) / 7
}
func sozEtcZones(x uint64) (n int) {
	return sovEtcZones(uint64((x << 1) ^ uint64((int64(x) >> 63))))
}
func (m *Zone) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 2:
			...
			if intStringLen < 0 {
				return ErrInvalidLengthEtcZones
			}
		...
		default:
			iNdEx = preIndex
			skippy, err := skipEtcZones(dAtA[iNdEx:])
			if err != nil {
				return err
			}
			if skippy < 0 {
...
func skipEtcZones(dAtA []byte) (n int, err error) {
	l := len(dAtA)
	iNdEx := 0
	depth := 0
	for iNdEx < l {
		...
		for shift := uint(0); ; shift += 7 {
			if shift >= 64 {
				return 0, ErrIntOverflowEtcZones
			}
			if iNdEx >= l {
		...
		case 2:
			...
			if length < 0 {
				return 0, ErrInvalidLengthEtcZones
			}
			iNdEx += length
		case 3:
			depth++
		case 4:
			if depth == 0 {
				return 0, ErrUnexpectedEndOfGroupEtcZones
			}
			depth--
		case 5:
			iNdEx += 4
...
var (
	ErrInvalidLengthEtcZones        = fmt.Errorf("proto: negative length found during unmarshaling")
	ErrIntOverflowEtcZones          = fmt.Errorf("proto: integer overflow")
	ErrUnexpectedEndOfGroupEtcZones = fmt.Errorf("proto: unexpected end of group")
)
